...
let _ = 
  Arg.parse 
    [
     ...
     "-o", Arg.String (fun s -> outfile := s), (sprintf "FILE\tdump result into FILE (default:SRC%s)" Astf.astf_ext);
...
*)
	...
	if !compile_mode then
      ...
      if !outfile = "" then
        if !filename = "" then
          Astf.dump_in_xml ast#root
        else
          Astf.save_in_xml (!filename^Astf.astf_ext) ast#root
      else
        Astf.save_in_xml !outfile ast#root;
      ...
      if na > 0 then
        fprintf stderr "ambiguous nodes: %d/%d\n" na ast#size;
