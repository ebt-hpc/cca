...
import sourcecode_metrics_for_survey_cpp as metrics
    ...
    def extract_metrics(self):
        if not self._metrics:
            ...
            try:
                ...
                self.message('fop_tbl has {} items'.format(len(fop_tbl)))
                # for k in fop_tbl.keys():
                #     print('!!! %s' % (k,))
                aa0_tbl = self._metrics.get_item_tbl(metrics.N_A_REFS[0])
                self.message('aa0_tbl has {} items'.format(len(aa0_tbl)))
