...
import sourcecode_metrics_for_survey_base as metrics
        ...
        if children == [] and callees_tbl:
            ...
            if callee_name:
                ...
                if callees:
                    ...
                    dp.debug('!!! not found: "{}"'.format(callee_name))

    scan(data)

class NodeBase(dp.base):
    ...
    def to_dict(self, ancl, ntbl,
        ...
        if self.cats & self.SUBPROGS:
            ...
            if lv > 0:
                ...
                self.debug('[REG] %s:%s:%d' % (ancl[0].get_name(), self.get_name(), lv))
        ...
        if is_caller:
            ...
            def filt(c):
                ...
                self.debug('[LOOKUP] %s:%s:%s' % (self.get_name(), c, len(ancl_)))
                hit = (self.get_name(), c, len(ancl_)) in ntbl

                b = b and (not hit)

                self.debug('[FILT] (%s->)%s (lv=%d) --> %s (hit=%s, max_lv=%d)' % (self.get_name(),
                                                                                   c.get_name(),
                                                                                   lv_,
                                                                                   b,
                                                                                   hit,
...
class OutlineBase(dp.base):
    ...
    def gen_data(self, lang, outdir='.', extract_metrics=True, omitted=set(), all_roots=False):
        ...
        self.message('%d root nodes found' % count)
            ...
            if ensure_dir(lver_dir):
                ...
                for json_d in json_ds:
                    ...
                    if ensure_dir(lver_loc_dir):
                        ...
                        self.debug('idx={}'.format(idx))
                        self.debug('lmi={}'.format(lmi))
                        if idx and lmi:
