    ...
    for (proj, vtbl) in ptbl.items():
        ...
        if os.path.exists(pdir):
            if not os.path.isdir(pdir):
                dp.warning('not a directory: "%s"' % pdir)
                continue
        else:
            os.makedirs(pdir)
...
def predict_kernels(fname, clf_path, model='minami', filt={},
    ...
    try:
        ...
        if data:
            ...
            dp.message('predicted %d kernels' % count)
...
def sample(fname, nsamples,
    ...
    try:
        ...
        dp.message('%d rows found' % len(rows))
        ...
        dp.message('%d projects found' % len(projs))
        ...
        dp.warning(fmt % (len(to_be_deleted),
                          bf0_thresh_upper,
                          bf1_thresh_upper,
                          bf2_thresh_upper,
                          bf0_thresh_lower,
        ...
        dp.message('%d rows' % len(rows))

        projs = set()
        for row in rows:
            projs.add(row[proj_i])

        dp.message('%d projects' % len(projs))
        ...
        dp.message('%d samples extracted' % len(samples))
